<div class="bg-white border border-gray-200 rounded-lg shadow-lg">
    ...
    @if(!empty($studentFatherContact))
        ...
        <div class="bg-indigo-600 py-4 px-6 flex items-center justify-between">
            <h1 class="text-xl text-white font-semibold">Send SMS to {{$studentFatherContact}}</h1>
                ...
                <div class="space-y-2">
                    <label class="relative flex items-center">
                        <input type="radio"
                               ...
                               name="smsLanguage"
                               value="English"
                               wire:model="smsLanguage"
                               class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer">
                        <span class="ml-8">English</span>
                    </label>
                    <label class="relative flex items-center">
                        <input type="radio"
                               id="urdu"
    ...
    @if($create)
        ...
        <form class="py-6 px-4 sm:px-6" wire:submit.prevent="updateProfile" enctype="multipart/form-data">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label for="diploma_id" class="block text-sm font-medium text-gray-700 mb-1">Admitted In
                        Diploma</label>
                    <div class="relative">
                        <select name="type" id="diploma_id" wire:model="diploma_id"
                            ...
                            @foreach($diplomaList as $diploma)
                                <option
                                    value="{{$diploma->id}}" @selected($diploma->id === $diploma_id)>{{$diploma->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="type" id="section_id" wire:model="section_id"
                            ...
                            @foreach($sectionList as $sec)
                                <option
                                    value="{{$sec->id}}" @selected($diploma->id === $diploma_id)>{{$sec->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="type" id="session_id" wire:model.live="session_id"
                            ...
                            @foreach($sessionList as $session)
                                <option
                                    value="{{$session->id}}" @selected($session_id === $session->id)>{{$session->name}}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    <label for="reg_no" class="block text-sm font-medium text-gray-700 mb-1">Assign Registration
                        #</label>
                ...
                <div>
                    <label for="father_contact" class="block text-sm font-medium text-gray-700 mb-1">Father / Guardian
                        Contact
                        #</label>
                ...
                <div>
                    <label for="postal_address" class="block text-sm font-medium text-gray-700 mb-1">Postal
                        Address</label>
                    ...
                    <div class="relative">
                        <select name="province_id" id="province_id" wire:model="province_id"
                            ...
                            @foreach($provinceList as $list)
                                <option
                                    value="{{$list->id}}" {{$this->province_id === $list->id? 'selected' : ''}}>{{$list->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="district_id" id="district_id" wire:model="district_id"
                            ...
                            @foreach($districtList as $dis)
                                <option
                                    value="{{$dis->id}}" {{$this->district_id === $dis->id? 'selected' : ''}}>{{$dis->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="blood_group_id" id="blood_group_id" wire:model="blood_group_id"
                            ...
                            @foreach($bloodGroupList as $bg)
                                <option
                                    value="{{$bg->id}}" {{$this->blood_group_id === $bg->id? 'selected' : ''}}>{{$bg->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="gender" id="gender" wire:model="gender_id"
                            ...
                            @foreach($genderList as $gen)
                                <option
                                    value="{{$gen->id}}" {{$this->gender_id === $gen->id? 'selected' : ''}}>{{$gen->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="religion" id="religion" wire:model="religion"
                            ...
                            @foreach($religionList as $religion)
                                <option
                                    value="{{ $religion->name }}" {{ $this->religion === $religion->value ? 'selected' : '' }}>
                                    {{ $religion->value }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="bg-indigo-500 py-4 px-6 flex items-center justify-between">
            ...
            <div>
                ...
                <a target="_blank" href="{{ route('student-card') }}"
                   class="inline-block ml-2 px-3 text-white py-1 bg-indigo-800 rounded-lg hover:bg-indigo-700 transition duration-300">
                    Student Id Cards
                </a>
        ...
        <div class="mt-5 px-8">
            <div class="flex justify-between">
                ...
                <div class="p-4">
                    ...
                    <label for="section_search" class="block text-sm font-medium text-gray-700 mb-1">Search By
                        Section</label>
                    <div class="section_search">
                        <select name="section_search" id="section_search" wire:model.live="section_search"
                                class=" block appearance-none w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            ...
                            <td class="px-4 py-2">
                                <div class="flex h-full items-center">
                                    ...
                                    <x-button
                                        ...
                                        wire:click="sendingSMS({{$student->id}})" title="Send SMS "
                                              wire:loading.attr="disabled">
                                        <i class="fas fa-envelope"></i>
                                    </x-button>
                                    <a target="_blank" href="{{ route('student-card', ['id' => $student->id]) }}"
                                       class="inline-block ml-2 px-3 text-white py-1 bg-red-500 rounded-lg hover:bg-indigo-700 transition duration-300">
